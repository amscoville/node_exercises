...
function isValidCard(number){
    if (isValidCardFormat(number)){
        var secondNumBy2 = multiplyBy2(number);
        var sumOfDigits = stringDigitSum(secondNumBy2);
        var finalSum = getSum(sumOfDigits);
        if(finalSum % 10 === 0){
            return true;
        }
        else{
...
function multiplyBy2(string) {
    var cardNumberArr = makeArray(string);
    for(var i = (cardNumberArr.length - 2); i >= 0; i-=2){
        cardNumberArr[i] *= 2;
    }
    return cardNumberArr;
}

function stringDigitSum(array) {
    for (var i = 0; i < array.length; i++){
        if(array[i] >= 10){
            var numToString = array[i].toString();
            var whatever = makeArray(numToString);
            var whoCares = getSum(whatever);
            array[i] = whoCares;
        }
    }
    return array;
